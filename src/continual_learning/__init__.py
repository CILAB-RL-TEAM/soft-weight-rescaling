...
def get_dataloader(
    ...
    train_chunk_size = n_train_data // n_chunks

    trainloaders = []
    for i in range(n_chunks):
        train_start_idx = i * train_chunk_size if access == 'limited' else 0
        chunk_train_indices = train_indices[train_start_idx:(i + 1) * train_chunk_size]
        subset = torch.utils.data.Subset(trainset, chunk_train_indices)
